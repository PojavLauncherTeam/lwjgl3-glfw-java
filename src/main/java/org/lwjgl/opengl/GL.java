...
public final class GL {
    ...
    private static GLXCapabilities initCapabilitiesGLX(boolean client) {
		...
		// nXOpenDisplay(NULL);
        try {
            return createCapabilitiesGLX(display, client ? -1 : 0 /* XDefaultScreen(display) */);
        } finally {
        ...
        try {
            ...
            try (MemoryStack stack = stackPush()) {
                ...
                if (callI(GetError) == GL_NO_ERROR && 4 < majorVersion) {
					...
					majorVersion = 2;
					minorVersion = 1;
				} else if (callI(GetError) == GL_NO_ERROR && 3 <= (majorVersion = version.get(0))) {
