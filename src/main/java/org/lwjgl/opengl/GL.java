...
public final class GL {
    ...
    public static void create() {
        SharedLibrary GL = new SharedLibrary() {
			@Override
			...
			public String getPath() {
				return "/data/data/net.kdt.pojavlaunch/lib/libgl04es.so";
			}
    ...
    private static GLXCapabilities initCapabilitiesGLX(boolean client) {
        long display = 1; // nXOpenDisplay(NULL);
        try {
    ...
    public static GLCapabilities createCapabilities(boolean forwardCompatible) {
        ...
        try {
            ...
            try (MemoryStack stack = stackPush()) {
				...
				if (glGetError() == GL_NO_ERROR && 3 <= (majorVersion = version.get(0))) {
                    // We're on an 3.0+ context.
					minorVersion = glGetIntegerv(GL_MINOR_VERSION);
                } else {
                    // Fallback to the string query.
                    String versionString = glGetString(GL_VERSION);
                    if (versionString == null || glGetError() != GL_NO_ERROR) {
